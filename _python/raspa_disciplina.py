    ...
    for span_tag in soup('span'):
        ...
        try:  # dados da disciplina em portugues
            if span_tag['class'][0] == 'txt_arial_8pt_gray':
                # por causa de alguma cagada que fizeram nos Objetivos da disciplina no Jupiter
                if span_tag('pre'):
...
def raspa_oferecimento(codigo):
    ...
    for d in div:
        ...
        keys = table[0].find_all(
            'span', class_="txt_arial_8pt_black")  # rótulos
        ...
        keys = table[1].find_all(
            'span', class_="txt_arial_8pt_black")  # rótulos
        items = table[1].find_all('span', class_="txt_arial_8pt_gray")

        turma['aulas'] = {}
        for h in range(len(items)//4):
            horario = f'{items[0+4*h].string} {items[1 +
                                                     4*h].string} {items[2+4*h].string}'
        ...
        keys = table[2].find_all(
            'span', class_="txt_verdana_8pt_white")  # rótulos
        items_total = table[2].find_all(
            'span', class_="txt_arial_8pt_black")  # total
        items_curso = table[2].find_all(
            'span', class_="txt_arial_8pt_gray")  # por turma
